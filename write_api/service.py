...
db = mysql.connector.connect(
    host="host.minikube.internal", user="root", password="admin123", database="twitter"
)
...
def insert_tweet():
    ...
    user_id = user[0]
    cursor.execute(
        "INSERT INTO tweets (body, user_id) VALUES (%s, %s)", (tweet_body, user_id)
    )
    db.commit()
    tweet_id = cursor.lastrowid

    # Add the tweet data to the RabbitMQ queue
    message = {"id": tweet_id, "user_id": user_id, "body": tweet_body}
